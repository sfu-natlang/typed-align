...
def generateFeature(f,i,e,j):
	left2_cn = e[j-2] if j>=2 else 'null#null'
	left1_cn = e[j-1] if j>=1 else 'null#null'
	right1_cn = e[j+1] if j<len(e)-1 else 'null#null'
	right2_cn = e[j+2] if j<len(e)-2 else 'null#null'
	left1_en = f[i-1] if i>=1 else 'null_null'
	right1_en = f[i+1] if i<len(f)-1 else 'null_null'
	#print left2_cn,left1_cn,e[j],right1_cn,right2_cn,left1_en,f[i],right1_en
	return {'Left2_cn':[left2_cn.split('#')[0]], 'POS_Left2_cn':[left2_cn.split('#')[1]], 'Left1_cn':[left1_cn.split('#')[0]], 'POS_Left1_cn':[left1_cn.split('#')[1]], 'Chinese':[e[j].split('#')[0]], 'POS_Chinese':[e[j].split('#')[1]], 'Right1_cn':[right1_cn.split('#')[0]], 'POS_Right1_cn':[right1_cn.split('#')[1]], 'Right2_cn':[right2_cn.split('#')[0]], 'POS_Right2_cn':[right2_cn.split('#')[1]], 'Left1_en':[left1_en.split('_')[0]], 'Prefix_Left1':[left1_en.split('_')[0][:5]], 'POS_Left1':[left1_en.split('_')[1]], 'English':[f[i].split('_')[0]], 'Prefix':[f[i].split('_')[0][:5]], 'POS':[f[i].split('_')[1]], 'Right1_en':[right1_en.split('_')[0]], 'Prefix_Right1':[right1_en.split('_')[0][:5]], 'POS_Right1':[right1_en.split('_')[1]]}
...
#"Computes the best alignment from a model and two sentences."
		...
		#link = []
		for i, f_i in enumerate(f):
			#j, p, l = argmax([(j, model.p(e_j, f_i)*model.p(e_j, f_i)*max(LogitRegModel.predict_proba(featureVec)[0]), LogitRegModel.predict(featureVec)[0]) for j, e_j in enumerate(e) for featureVec in [vectorModel.transform(concatenate(f_i, e_j, flag))] ])
			j, p, l = argmax([(j, model.p(e_j.split('#')[0], f_i.split('_')[0])*max(LogitRegModel.predict_proba(featureVec)[0]), LogitRegModel.predict(featureVec)[0]) for j, e_j in enumerate(e) for featureVec in [vectorModel.transform(generateFeature(f,i,e,j))] ])
...
def main(mode):
	if mode == "train":
		...
		print "loading alignment model..."
		model = pickle.load(open('viterbi.20k.wordNoLink.cn-en', 'rb'))
		english, french = read_corpus('train.20k.seg.cln.pos.cn.part9', 'train.20k.seg.cln.pos.en.part9')
		...
		print "finding best alignment..."
		for s, (e, f) in enumerate(zip(english, french), 19569):
			if s%100 == 0:
				#print "calculate sentence pair #", s
                		sys.stdout.flush()
			alignment = alignWithLogitReg(model, e, f, combined, logitRegModel, True)
			for i, (a_i, l) in enumerate(alignment, 1):
				print s, a_i, i, l
	elif mode == "intersect":
		...
		for line in chin:
			...
			for word in words:
				if (word.isalnum() or len(word)%3!=0):
					...
					continue
				chars = list()
				for i in range(int(index),int(index+len(word)/3)):
