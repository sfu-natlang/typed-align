...
def EM(counter, model, iterations = 10):
		...
		for s, (e, f) in enumerate(self.bitext):
			...
			for i, f_i in enumerate(f):
				...
				for j, e_j in enumerate(e):
					...
					inc(counts.word, e_j, 1.0)
					inc(counts.words, (e_j, f_i), 1.0)
...
def main(mode):
	if mode == "train":
		english, french = read_corpus('train.20k.word.cn', 'train.20k.en')
		counter = Counter(english, french)
		model1 = Model1(counter.init_model())
		model1 = EM(counter, model1)
		out_model = open('viterbi.20k.10iter.wordNoLink.cn-en', 'wb')
		pickle.dump(model1, out_model, pickle.HIGHEST_PROTOCOL)
	elif mode == "align":
		print "loading alignment model..."
		model = pickle.load(open('viterbi.20k.10iter.wordNoLink.cn-en', 'rb'))
		english, french = read_corpus('train.20k.word.cn', 'train.20k.en')
		...
		for line in align:
			...
			if (num != linum):
				if(num!=linum+1):
					...
					linum = linum + 1
					giza.write("\n")
			a = str(int (line.split()[1])) + "-" +  str(int (line.split()[2])) + '(' + line.split()[3] + ')'
			giza.write(a)
			giza.write(" ")
