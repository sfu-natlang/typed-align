						...
						if engIndices[0].strip():
							for wordIndex in engIndices:
								...
								if (chinWord, engWord) in model.counts.words:
									delta = model.p(chinWord,engWord) / total
									inc(counts.words, (chinWord,engWord), delta)
									inc(counts.word, chinWord, delta)
					else:
						left = alm.split('-')[0]
						if isContiguous(left):
							#chinWord = ""
...
def main(mode):
	if mode == "train":
		english, french, gold_alignment = read_corpus('train.20k.seg.cln.cn', 'train.20k.seg.cln.en', 'train.20k.seg.cln.wa')
		counter = Counter(english, french, gold_alignment)
		model1 = Model1(counter.init_model())
		model1 = EM(counter, model1)
		out_model = open('model.20k.gold.NoLink.cn-en', 'wb')
